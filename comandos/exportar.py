...
def export_vendas():
    ...
    if escolha == 2: #--------------------------------------------Escolheu Imprimir Todos
        ...
        print("----------SUCESSO")
    if escolha == 1: #--------------------------------Escolheu Imprimir Quantidade Específica
        try:
        ...
        try:
            ...
            faturamento_mensal = sum(venda[3] for venda in vendas if str(venda[0]).split("-")[1] ==  mes_atual)
            faturamento_diario = sum(venda[3] for venda in vendas if str(venda[0]).split("-")[2] ==  dia_atual)
        except mysql.connector.Error as erro:
            print(f"\nArquivo: exportar - Linha: 39\nERRO: Falha no Banco de Dados, {erro}")
            input("Aperte ENTER para Continuar")
            return
